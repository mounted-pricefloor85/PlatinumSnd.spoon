...
function Probe:roleAt(x, y)
  ...
  local ok, element = pcall(hs.axuielement.systemElementAtPosition, x, y)
  local elapsed = hs.timer.secondsSinceEpoch() - started
  self.breaker:noteProbeTime(elapsed, now)
  ...
  pcall(function() element:setTimeout(self.tuning.axTimeoutSeconds) end)
